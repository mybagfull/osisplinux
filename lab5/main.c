...
void init(void) {
  ...
  if ((free_space = sem_open("free_space", (O_RDWR | O_CREAT | O_TRUNC), (S_IRUSR | S_IWUSR), start_Max))
      == SEM_FAILED ||
...
void msg_queue_init() {
  queue.curr_Max = start_Max;
}

int put_msg(msg* _msg) {
  if (queue.msg_count == queue.curr_Max) {
    fprintf(stderr, "Queue buffer overflow\n");
    exit(1);
  }

  if (queue.tail == queue.curr_Max) {
    queue.tail = 0;
  }
...
void dec()
{
  ...
  pthread_mutex_unlock(&mutex);
    printf("%ld consume msg:extract_count=%d\n",
