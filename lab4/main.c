...
void init(void) {
  ...
  void* ptr = mmap(NULL, sizeof(msg_queue), PROT_READ | PROT_WRITE,MAP_SHARED, fd, 0); // map_shared - other processes can see it 
  if (ptr == MAP_FAILED) {
    fprintf(stderr, "ftruncate");
    exit(1);
  }
...
void remove_producer() {
  ...
  --producers_amount;
  kill(producers[producers_amount], SIGUSR1);
  wait(NULL);
}
...
void remove_consumer() {
  ...
  consumers_amount--;
  kill(consumers[consumers_amount], SIGUSR1);
  wait(NULL);
}
